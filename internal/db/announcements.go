...
// CreateAnnouncement persists a draft broadcast.
func (s *AnnouncementStore) CreateAnnouncement(ctx context.Context, a notifications.Announcement) (*notifications.Announcement, error) {
	...
	created, err := scanAnnouncement(s.pool.QueryRow(ctx, q,
...
// ListSegmentSubscriberIDs resolves an announcement's segment to the
// subscriber ids it targets.
//
// Each NULL filter means "no restriction on this dimension", so an
// announcement with all three unset addresses the whole base. Terminated
// subscribers are excluded unconditionally: they have left, and a marketing
// broadcast to a former customer is at best noise and at worst a compliance
// problem.
func (s *AnnouncementStore) ListSegmentSubscriberIDs(ctx context.Context, franchiseID, planID *int, status *string) ([]int, error) {
	const q = `
