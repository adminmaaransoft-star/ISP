...
// WarningPayload is the Asynq task payload for 80% FUP warnings.
...
// ProcessTask implements asynq.Handler for TaskTypeFUPWarning.
func (h *WarningHandler) ProcessTask(ctx context.Context, t *asynq.Task) error {
	var p WarningPayload
	if err := json.Unmarshal(t.Payload(), &p); err != nil {
		// A malformed payload will never become valid on retry.
		return fmt.Errorf("fup warning: unmarshal payload: %w: %w", err, asynq.SkipRetry)
	}
	if h.notifier == nil {
